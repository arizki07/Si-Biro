...
class JadwalController extends Controller
{
    public function index(Request $request)
    {
        ...
        $grup = GrupModel::select('kode_grup', \DB::raw('MIN(id_grup) as id_grup'), \DB::raw('MIN(id_jamaah) as id_jamaah'), \DB::raw('MIN(id_layanan) as id_layanan'), \DB::raw('MIN(no_hp) as no_hp'))->groupBy('kode_grup')->get();
    ...
    public function send_whatsapp(Request $request, $id)
    {
        ...
        $Qgrup = DB::table('t_grup')
            ...
            ->where('id_layanan', $id_layanan) // ->get();
            ->pluck('no_hp');
        // echo "<pre>";
        // echo($Qgrup) . "<br><br><br>";
        // echo "</pre>";die;
        ...
        $Qjadwal = DB::table('t_uraian_jadwal')->select('*')->where('tahap', $tahap)->where('nomor_jadwal', $nomor_jadwal)->first();
        ...
        foreach ($Qgrup as $nomor_hp) {
            ...
            curl_setopt_array($curl, [
                ...
                CURLOPT_HTTPHEADER => ['Authorization: ZQFA!45KMw+YfkBK-CJj'],
            ]);
            ...
            if (!$response) {
                ...
                $wee = DB::table('t_grup')->select('*')->where('kode_grup', $kode_grup)->where('id_layanan', $id_layanan)->get();
        ...
        return redirect()
            ->back()
            ->with('success', 'Berhasil Memproses Whatsapp Jadwal ' . ucwords(strtolower($tipe_jadwal)) . ' Tahap ' . $tahap . ' Grup ' . $kode_grup);
    }
    ...
    public function upload_file(Request $request, $id)
    {
        ...
        $uraian = UraianJadwalModel::findOrFail($id);
        ...
        if ($request->hasFile('file')) {
            ...
            switch ($act) {
                case 'jadwal-mcu':
                    $filename = 'MCU_' . $uraian->judul_mcu . '_' . $timestamp . '.' . $extension;
                    $path = 'document/file-mcu/' . $filename;
                    $this->apaini($uraian->file_mcu, $path, $fileIN, $act);
                    $uraian->file_mcu = $filename;
                    break;

                case 'jadwal-passport':
                    $filename = 'PASS_' . $uraian->judul_passport . '_' . $timestamp . '.' . $extension;
                    $path = 'document/file-passport/' . $filename;
                    $this->apaini($uraian->file_passport, $path, $fileIN, $act);
                    $uraian->file_passport = $filename;
                    break;

                case 'jadwal-bimbingan':
                    $filename = 'BIM_' . $uraian->judul_bimbingan . '_' . $timestamp . '.' . $extension;
                    $path = 'document/file-bimbingan/' . $filename;
                    $this->apaini($uraian->file_bimbingan, $path, $fileIN, $act);
                    $uraian->file_bimbingan = $filename;
                    break;

                case 'jadwal-manasik':
                    $filename = 'MAN_' . $uraian->judul_manasik . '_' . $timestamp . '.' . $extension;
                    $path = 'document/file-manasik/' . $filename;
                    $this->apaini($uraian->file_manasik, $path, $fileIN, $act);
                    $uraian->file_manasik = $filename;
                    break;

                default:
                    return redirect()->back()->with('error', 'Tipe tidak diketahui.');
            }
        ...
        return redirect()->back()->with('error', 'File gagal diperbarui.');
    }

    private function apaini($oldFilename, $newPath, $file, $act)
    {
        if ($oldFilename) {
            switch ($act) {
                case 'jadwal-mcu':
                    Storage::disk('public')->delete('document/file-mcu/' . $oldFilename);
                    break;
                case 'jadwal-passport':
                    Storage::disk('public')->delete('document/file-passport/' . $oldFilename);
                    break;
                case 'jadwal-bimbingan':
                    Storage::disk('public')->delete('document/file-bimbingan/' . $oldFilename);
                    break;
                case 'jadwal-manasik':
                    Storage::disk('public')->delete('document/file-manasik/' . $oldFilename);
                    break;
            }
        }
