...
class UraianJadwalUpdateImport implements ToModel, WithStartRow
{
    /**
     * @param array $row
     *
    ...
    public function model(array $row)
    {
        ...
        // $cek = UraianJadwalModel::where('nomor_jadwal', $row[0])->exists();
        // if (!$cek) {
        //     throw new \Exception("Nomor Jadwal Pada Sheet URAIAN JADWAL Tidak Ditemukan.");
        // } 

        // $limit = UraianJadwalModel::where('flag_update', $row[16])->first();
        UraianJadwalModel::destroy($row[16]);
        // UraianJadwalModel::where('flag_update', $row[16])->delete();
